...
def get_cross_section_parameters_circular(d, E, fy):
...
def get_cross_section_parameters_tube(D, t, E, fy):
    """ Gets parameters for a hollow tube cross-section
    D: steel tube outer diameter [mm]
...
def get_p_f_elastoplastic_user(c_u, a):
    """ Gets stress from lateral soil resistance at yield (p-y curve)
    Linear relationship with constant a defined by the user
    """
    p_f = a*c_u
    return p_f # kN/m^2
...
def get_reduction_factor_chi(phi, lambda_):
    """ Gets the reduction factor psi"""
    chi = 1/(phi+(phi**2-lambda_**2)**0.5)
    return chi

def get_buckling_resistance(chi, fyA, gamma_M=1.1):
    """ Gets the cross-sectional buckling resistance"""
    Nb_Rd = chi*fyA/gamma_M
    return Nb_Rd
...
def get_Nb_Rd_EC3(fyA, Ncr, buckling_curve='c', gamma_M=1.1):
    ...
    chi = get_reduction_factor_chi(phi, lambda_)
    Nb_Rd = get_buckling_resistance(chi, fyA, gamma_M)
    return Nb_Rd, lambda_, chi
...
def get_Nb_Rd_DIBt(L, D, EI, fyA, c_u, buckling_curve='c', gamma_M=1.1):
    ...
    if c_u < 10:    # no lateral soil support
        Ncr = EI*(np.pi/L)**2
        Nb_Rd, _, _ = get_Nb_Rd_EC3(fyA, Ncr, buckling_curve, gamma_M)
    elif c_u <= 30:
        Nb_Rd, _, _ = get_Nb_Rd_EC3(fyA, Ncr, buckling_curve, gamma_M)
    else:
        Nb_Rd = fyA/gamma_M
...
def plot_cross_section(D, d):
    """ Plots cross-section of a micropile"""
    fig, ax = plt.subplots()
    circle_D = Circle((0, 0), D/2, facecolor='magenta', edgecolor='black', zorder=0, alpha=0.3, label='Grout d = {:.2f} mm'.format(D))
    circle_d = Circle((0, 0), d/2, facecolor='darkblue', edgecolor='black', zorder=1, alpha=1.0, label='Steel bar d = {:.2f} mm'.format(d))
...
def plot_cross_section_tube(D, t, D_grout):
    """ Plots cross-section of a duki-pile
    D: outer tube diameter
    t: tube thickness"""
    fig, ax = plt.subplots()
    circle_D = Circle((0, 0), D/2, facecolor='darkblue', edgecolor='black', zorder=1, alpha=1.0, label='Steel tube outer D = {:.2f} mm'.format(D))
    circle_d = Circle((0, 0), (D-2*t)/2, facecolor='magenta', edgecolor='black', zorder=2, alpha=0.7, label='Grout diameter = {:.2f} mm'.format(D-2*t))
    circle_D_grout = Circle((0, 0), (D_grout)/2, facecolor='aqua', edgecolor='black', zorder=0, alpha=0.3, label='Grout diameter = {:.2f} mm'.format(D_grout))
    ax.add_patch(circle_d)
    ax.add_patch(circle_D)
    ax.add_patch(circle_D_grout)
    ax.autoscale_view()
    ax.set_aspect('equal')
    ax.legend()
...
def display_micropile_buckling_resistance(D, EI, fyA, k_imp, L, p_f_eq, a, c_u, Nb_Rd, buckling_curve, gamma_M):
    """ Gets final results"""
    ...
    try:
        p_f_array = p_f_eq(c_u_array)    # p_f equation 2/ or 3/ or 4
        p_f = p_f_eq(c_u)
    except:
        p_f_array = p_f_eq(c_u_array, a) # p_f equation user
        p_f = p_f_eq(c_u, a)
    ...
    for i in range(Ncr_array.size):
        ...
        # get cross-sectional buckling resistance after EC3
        Nb_Rd_EC3_i, _, _ = get_Nb_Rd_EC3(fyA, Ncr_i, buckling_curve, gamma_M)
        Nb_Rd_DIBt_i = get_Nb_Rd_DIBt(L, D, EI, fyA, c_u_array[i], buckling_curve, gamma_M)
...
def display_duki_buckling_resistance(D, EI, fyA, k_imp, L, p_f_eq, a, c_u, Nb_Rd, buckling_curve, gamma_M):
    """ Gets final results"""
    # Data generation and plotting
    c_u_array = np.linspace(1, 120, 120)
    w_f_array = get_w_f_elastoplastic_eq1(D, c_u_array)
